  ...
  .actions((self) => {
    const editAsset = flow(function* (body: any) {
      const res = yield httpRequest.sendPut(
        `${Config.BASE_URL}/portfolio/${PortfolioDetailStore.information.id}/crypto/${self.id}`,
        body,
    ...
    const getTransactionList = flow(function* () {
      ...
      const res = yield httpRequest.sendGet(
        `${Config.BASE_URL}/portfolio/${PortfolioDetailStore.information.id}/crypto/${self.id}/transactions`,
        UserStore.user.token
      );
      if (res instanceof HttpError) {
